...
// newStore builds a store over a one-screening schedule, so a test never
// depends on the real Theatre service. Curtain-up is a full week out, which
// is the moment a house goes on sale: nothing has been sold in advance yet,
// so the test has every seat to itself.
func newStore(t *testing.T) *Store {
	t.Helper()
	startsAt := time.Now().Add(7 * 24 * time.Hour).UTC().Format(time.RFC3339)
	schedule := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		w.Write([]byte(`{"shows":[{"id":"a-film@a-house","movieId":"a-film","theaterId":"a-house","startsAt":"` + startsAt + `","priceCents":1000}],"total":1,"nextCursor":null}`))
	}))
	t.Cleanup(schedule.Close)
	return New(theatre.NewClient(schedule.URL))
}

func TestBookSellsTheSeats(t *testing.T) {
	s := newStore(t)

	booking, err := s.Book("a-film@a-house", []string{"E7", "E8"})
	if err != nil {
		t.Fatalf("Book: %v", err)
	}
	...
	seatMap, err := s.SeatMap("a-film@a-house")
	if err != nil {
...
func TestBookIsAllOrNothing(t *testing.T) {
	...
	if _, err := s.Book("a-film@a-house", []string{"E8"}); err != nil {
		t.Fatalf("first Book: %v", err)
	}

	_, err := s.Book("a-film@a-house", []string{"E7", "E8", "E9"})
...
func TestBookRefusesWhatTheHouseDoesNotSell(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			if _, err := s.Book("a-film@a-house", test.seats); status.Code(err) != test.want {
...
func TestHeldSeatsCannotBeBought(t *testing.T) {
	...
	holdID, err := s.Hold("a-film@a-house", []string{"E7"})
	if err != nil {
		t.Fatalf("Hold: %v", err)
	}

	if _, err := s.Book("a-film@a-house", []string{"E7"}); status.Code(err) != codes.AlreadyExists {
	...
	if _, err := s.Book("a-film@a-house", []string{"E7"}); err != nil {
